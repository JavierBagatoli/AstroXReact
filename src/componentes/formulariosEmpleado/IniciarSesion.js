...
import { contraseñaFomato, mailValido } from "../../helpers/validarEntradas";
...
const IniciarSesion = ({ handleLogin }) => {
  ...
  const validar = () => {
    ...
    if (contraseñaFomato(passwordRef.current.value)) {
      vectorErrores =
        "Contraseña repetida no valido, debe contener al menos 6 caracteres, usar minusculos, mayusculas y simbolos";
    }
    ...
    <article>
      <div className="articulo login">
        ...
        <div className="columna">
          <input
            ref={mailRef}
            className="input-agregar-tarea c1"
            type="mail"
            placeholder="Correo"
          />
          <input
            ref={passwordRef}
            className="input-agregar-tarea c2"
            type="password"
          ...
          <button
            ...
            className="boton c3 boton-centrar"
          >
            Iniciar sesión
          </button>
        </div>
